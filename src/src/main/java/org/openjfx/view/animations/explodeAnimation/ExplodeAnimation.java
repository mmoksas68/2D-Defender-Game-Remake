package org.openjfx.view.animations.explodeAnimation;
...
public class ExplodeAnimation {
    ...
    public ExplodeAnimation(ModelToView modelToView){
        ...
        {
            try {
                imageViewList[i] = new ExplosionStage(new Image(new FileInputStream(imageUrl + (i+1) + ".png" )));
                imageViewList[i].setFitHeight(modelToView.getHitboxHeight());
                imageViewList[i].setFitWidth(modelToView.getHitboxWidth());
                imageViewList[i].setTranslateX(modelToView.getLocationX() - modelToView.getCurrentViewLeft());
